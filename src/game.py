...
class MainGame(GameScene):
    ...
    def on_switchto(self, switchtoargs):
        ...
        for xi in range(math.floor(self.course_len * self.window_size[0] / 1)):
            ...
            x += step
            self.wavesurface.blit(resources.kernal_gfx,
                                  (self.window_size[0] * (x + 0.5) - 8, self.window_size[1] * y - 8))
        y0 = self.wave.wave_func(0.0)
        self.wavesurface.blit(resources.kernal_end_gfx,
                              (self.window_size[0] * 0.5 - 16, self.window_size[1] * y0 - 16))
        yend = self.wave.wave_func(self.course_len)
        self.wavesurface.blit(resources.kernal_end_gfx,
                              (self.window_size[0] * (self.course_len + 0.5) - 16, self.window_size[1] * yend - 16))
    ...
    def on_update(self):
        ...
        if self.countin > 0:
            ...
            if not self.exiting:
                ...
                if 0 < slope < self.player_angle:  # set to zero if going right direction
                    diverge = 0
                if 0 > slope > self.player_angle:  # set to zero if going right direction
                    diverge = 0
                ...
                accel = 0.005
                drag = 10.0
                self.player_vel = self.player_vel - drag * self.player_vel * self.player_vel + accel * flow
    ...
    def on_event(self, events):
        for event in events:
            ...
            if event.type == KEYDOWN or event.type == KEYUP:
                self.control.process_key_event(event)
    ...
    def draw(self, screen):
        ...
        par_pos = (self.window_size[0] / 2 - self.window_size[0] * (self.player_pos - self.par_position),
                   self.window_size[1] * self.wave.wave_func(self.par_position))
